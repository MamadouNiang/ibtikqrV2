...
export class CommRecenssementComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.listeRegions = JSON.parse(localStorage.getItem('regions'));
  ...
  importCsv(e) {
   ...
   this.regionId = this.listeRegions.filter((e,id) =>  e.name === this.region);
  ...
  validerImp() {
    ...
    if (e.target.files.length > 0) {
      ...
      formData.append('regionId', this.regionId[0].id);
      formData.append('campaignId', this.validerFormCampagne.get('idCamp').value);
